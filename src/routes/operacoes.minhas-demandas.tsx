...
function MinhasDemandas() {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
           <div className="relative w-64">
              ...
              <Input 
                ...
                className="h-9 pl-9 text-[12.5px] bg-muted/40 border-border" 
                value={search}
                onChange={e => setSearch(e.target.value)}
              />
           </div>
           <Button variant="outline" size="sm" className="h-9 gap-2 text-[12px] border-border bg-muted/40">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3 rounded-xl border border-border bg-muted/30 p-4">
        <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Ver demandas de</span>
        <Select
          value={effectiveUser?.id ?? ''}
          onValueChange={(v) => setManualUserId(v)}
        >
          <SelectTrigger className="h-9 w-64 text-[12.5px] bg-muted/40 border-border">
      ...
      {!effectiveUser ? (
        <div className="rounded-2xl border border-dashed border-border p-12 text-center">
          <p className="text-muted-foreground">Selecione um colaborador acima para ver suas demandas.</p>
        </div>
      ) : tasks.length === 0 ? (
            ...
            return (
              <motion.div 
                ...
                className={`group relative overflow-hidden rounded-2xl border transition-all duration-300 ${
                  isExpanded ? 'ring-2 ring-border shadow-2xl' : 'hover:shadow-lg'
                } ${
                  isOverdue 
                ...
                >
                  ...
                  <div className="flex items-center gap-8 shrink-0">
                    <div className="text-right hidden sm:block">
                      <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Prazo</div>
                      <div className={`font-mono text-[13px] font-medium tabular-nums ${isOverdue ? 'text-red-400' : 'text-foreground'}`}>
                    ...
                    <div className="flex items-center gap-2">
                       {t.checklist.length > 0 && (
                         <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-muted/50 border border-border/60 text-[11px] font-mono text-muted-foreground">
                           <ListChecks className="h-3.5 w-3.5" />
                           {done}/{t.checklist.length}
                         </div>
                       )}
                       <div className={`p-2 rounded-lg transition-transform ${isExpanded ? 'rotate-180 bg-muted' : 'bg-transparent text-muted-foreground'}`}>
                      ...
                      <div className="p-6 grid grid-cols-1 md:grid-cols-12 gap-8">
                        ...
                        <div className="md:col-span-8 space-y-6">
                          <div>
                             <div className="flex items-center justify-between mb-4">
                                <h4 className="text-[13px] font-semibold text-muted-foreground uppercase tracking-widest flex items-center gap-2">
                             ...
                             <div className="space-y-2.5">
                                {t.checklist.length === 0 ? (
                                  <p className="text-[13px] italic text-muted-foreground">Nenhum item definido.</p>
                                ) : (
                          ...
                          <div className="flex flex-wrap items-center gap-3">
                             ...
                             >
                               <SelectTrigger className={`h-10 w-48 text-[12px] font-medium border-border ${meta.color}`}>
                                 <SelectValue />
                               </SelectTrigger>
                               <SelectContent>
                             ...
                             <Button 
                               variant="outline" 
                               className="h-10 border-border bg-muted/40 gap-2 text-[12.5px]"
                               asChild
                             >
                        ...
                        <div className="md:col-span-4 space-y-6">
                           <div className="rounded-2xl border border-border/60 bg-muted/30 p-5 space-y-4">
                              <div>
                                 <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Prioridade</div>
                                 <div className="flex items-center gap-2">
                                    <div className={`h-2 w-2 rounded-full ${t.priority === 'alta' ? 'bg-red-500' : t.priority === 'media' ? 'bg-yellow-500' : 'bg-blue-500'}`} />
                                    <span className="text-[13px] capitalize text-foreground">{t.priority}</span>
                                 </div>
                              </div>
                              <div>
                                 <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Início</div>
                                 <div className="text-[13px] text-foreground">{t.startDate ? formatLocalDate(t.startDate) : '—'}</div>
                              </div>
                              {t.comments && t.comments.length > 0 && (
                                <div>
                                   <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-2 flex items-center gap-1.5">
                                   ...
                                   <div className="space-y-3">
                                      {t.comments.slice(-2).map(c => (
                                        <div key={c.id} className="text-[12px] leading-relaxed">
                                           <span className="font-bold text-muted-foreground">{c.authorName || 'Membro'}:</span>{' '}
